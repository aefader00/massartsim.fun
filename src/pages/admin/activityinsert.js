...
export default function controlPanel() {
  const queryDatabase = (values) => {
    ...
    fetch("/api/insertActivity", options);
  };
    ...
    <Layout>
      ...
      <div>
        <h1 className={styles.heading}>
          <a>Insert a new Activity into the database.</a>
        </h1>
        <Formik
          ...
          onSubmit={(values, { resetForm }) => {
            ...
            alert(
              `Congratulations, you have sent a query to the database!\n\nWait a few minutes and then check to make sure the Activity has been updated properly. If it hasn't, contact Anthony.`
            );
            //resetForm();
          }}
          validate={(values) => {
