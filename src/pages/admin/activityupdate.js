...
export default function controlPanel({ activities }) {
  ...
  const queryDatabase = (values) => {
    ...
    fetch("/api/updateActivity", options);
  };

  const deleteFromDatabase = () => {
    var values = { id: activityId };
    const options = {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(values),
    ...
    fetch("/api/deleteActivity", options);
  };
    ...
    <Layout>
      ...
      <div>
        <h1 className={styles.heading}>
          <a>Update an Activity in the database.</a>
        </h1>
        <Formik
          ...
          {({ values, errors }) => (
            <Form>
              ...
              <h3 className={styles.heading}>
                <a>YOUR UPDATED ACTIVITY WILL LOOK LIKE THIS:</a>
              </h3>
        ...
        <br />
        <h1 className={styles.heading}>
          <a>Delete an Activity from the database.</a>
        </h1>
        {activities.map(
          ({
            id,
        ...
        <div style={{ margin: "0.5em" }}>
          ...
          >
            <a>Click here to delete the above Activity!</a>
          </button>
        </div>
      </div>
    </Layout>
